...
def compute_pose_errors(data, config):
    ...
    for bs in range(K0.shape[0]):
        ...
        if ret is None:
            ...
            # data['inliers'].append(np.array([]).astype(np.bool)) # rtx
            data['inliers'].append(np.array([]).astype(np.bool_)) #3090
        else:
            R, t, inliers = ret
            t_err, R_err = relative_pose_error(T_0to1[bs], R, t, ignore_gt_t_thr=0.00001)
...
def error_auc(errors, thresholds):
    ...
    for thr in thresholds:
        ...
        aucs.append(np.trapz(y, x) / thr) #近似auc，不同的error取阈值，小于其的比例为值积分。确实是有auc的意思，是对各个阈值下正确比例积分参考
