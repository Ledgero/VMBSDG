...
from romatch.src.datasets.tools import wandering_tool
import random
import time

from romatch.src.utils.dataset import read_megadepth_gray, read_megadepth_depth, read_megadepth_gray_forMixed, read_megadepth_depth_forMixed, resize_gray
...
class MegaDepthDataset(Dataset):
    def __init__(self,
        ...
        self.scene_info={}
        self.scene_infoBuf = dict(np.load(npz_path, allow_pickle=True))
        ...
        # 修正数据结构
        self.pair_infos = [(pair_info[0], (pair_info[1],)) if not isinstance(pair_info[1], tuple) else pair_info for pair_info in self.pair_infos]
        ...
        mixedTxtPath="/home/ozq/LoFT/LoFTR/data/megadepth/index/trainvaltest_list/test_list.txt" 
    ...
    def __getitem__(self, idx):
       ...
       # (idx0, idx1), overlap_score, central_matches = self.pair_infos[idx]  #这里改一下数据组织方式把，不知道原来的megadepth是怎么办到的。
        ...
        # if not os.path.exists(img_name0):
        #     print(f"Image file not found at path: {img_name0}")
        #     time.sleep(1)
        # if not os.path.exists(img_name1):
        #      print(f"Image file not found at path: {img_name1}")
        #      time.sleep(1)
        ...
        if mixed_Train==1 and self.mode=='train': 
            ...
            npz_BasePath_mixed="/home/yxz/ozq/LoFT/LoFTR/data/megadepth/index/scene_info_0.1_0.7/" ##scene_info_0.1_0.7_reduced4 scene_info_0.1_0.7_reduced1 scene_info_0.1_0.7
            root_dir_mixed="/home/yxz/ozq/LoFT/LoFTR/data/megadepth/train/" #换位置
            ...
            # MapX0 和 MapY0，它们表示从 img0 到 img1 的像素映射关系。 K_0 和 K_1 是相机内参矩阵。 T_0to1 是从 img0 到 img1 的相对姿态。
            ...
            # cv2.imwrite("/home/yxz/ozq/LoFT/LoFTR/logs/tb_logs/"+str(idx)+"img0_Mixed.jpg",img0_Mixed.astype(np.uint8))
            # cv2.imwrite("/home/yxz/ozq/LoFT/LoFTR/logs/tb_logs/"+str(idx)+"img1_Mixed.jpg",img1_Mixed.astype(np.uint8))
            # time.sleep(5)
