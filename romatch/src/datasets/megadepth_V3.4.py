...
class MegaDepthDataset(Dataset):
    def __init__(self,
        ...
        mixedTxtPath="/home/smf/Airsim/LoFTR/data/megadepth/index/trainvaltest_list/trainListReduced_2to100.txt"
        with open(mixedTxtPath,'r') as file:
            self.scene_infos_mixed = [line.rstrip('\n') for line in file]

        #随机找一个场景，先固定这个,其实也就固定一张照片（好像也能随机，反正就一次读取），保存这张照片和。（尽量保证照片遍历到）
    ...
    def __getitem__(self, idx):
       ...
       # (idx0, idx1), overlap_score, central_matches = self.pair_infos[idx]  #这里改一下数据组织方式把，不知道原来的megadepth是怎么办到的。
        ...
        if mixed_Train==1 and self.mode=='train': 
            ...
            #加个设置可选，如果选mix，这里更新imga0 1 depth0 1, 不管怎么样都会保存 real RTK 01。先在gemotry里改
            #图像缩放mix这里做，更新image depth
            #相关路径规整成变量，从外部读取
            npz_BasePath_mixed="/home/smf/Airsim/LoFTR/data/megadepth/index/scene_info_0.1_0.7_reducedALL/scene_info_0.1_0.7_reduced1/" #先在这里搞个小test
            root_dir_mixed="/home/smf/Airsim/LoFTR/data/megadepth/train/" #换位置
            ...
            idx_mixed=0 #这里也加随机,如果__getitem__不能随机的话，那就是固定idx_mixed%368之类的, 先实验跑跑
            ...
            #reScaleBase = [0.5, 0.8] #分别给pair 0，1，之后这个搞到外面的参数 rScale0_mixed>1
            reScaleBase = [0.5, 0.5] #分别给pair 0，1，之后这个搞到外面的参数 rScale0_mixed>1
            ...
            # print("00img0_Mixed ",img0_Mixed.shape)
            # print("00mg1_Mixed ",img1_Mixed.shape)
            ...
            # print("11img0_Mixed ",img0_Mixed.shape)
            # print("11mg1_Mixed ",img1_Mixed.shape)
            # depth0_Mixed=depth0.copy() #这里相当于已经pad了，不需要再pad处理了,直接用depth0
            # depth1_Mixed=depth1.copy()
            depth0_Mixed=depth0
            depth1_Mixed=depth1
