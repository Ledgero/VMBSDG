...
from romatch.datasets.megadepth import MegadepthScene
...
from romatch.src.utils.augment import build_augmentor
from romatch.src.utils.dataloader import get_local_split
from romatch.src.utils.misc import tqdm_joblib
from romatch.src.utils import comm
from romatch.src.datasets.megadepth import MegaDepthDataset
from romatch.src.datasets.scannet import ScanNetDataset
from romatch.src.datasets.sampler import RandomConcatSampler
...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def __init__(self, args, config):
        ...
        # self.modality_list = None if not args.modality_list else  args.modality_list
        ...
        try:
            ...
            self.world_size = 1
            self.rank = 0
            logger.warning(str(e) + " (set wolrd_size=1 and rank=0)")
    ...
    def _setup_dataset(self,
                       ...
                       mode='train',
                       min_overlap_score=0.,
                       pose_dir=None,modality_list=None,**kwargs):
        ...
        return dataset_builder(data_root, local_npz_names, split_npz_root, intri_path,
                                mode=mode, min_overlap_score=min_overlap_score, pose_dir=pose_dir,modality_list=modality_list,**kwargs)
    # 将多个子数据集合并成一个数据集
            ...
            if data_source == 'ScanNet':
                datasets.append(
                    ScanNetDataset(data_root,
                                   ...
                                   pose_dir=pose_dir))
            elif data_source == 'MegaDepth' and not kwargs.get('loftr', False):
                #logger.info(f'[rank:{self.rank}] loading {mode} datasets in original way')
                datasets.append(
                ...
                import numpy as np
                #logger.info(f'[rank:{self.rank}] loading {mode} datasets in roma way')
                scene_info = dict(np.load(npz_path, allow_pickle=True))
                datasets.append(
                    MegadepthScene(
                        data_root, scene_info, min_overlap=min_overlap_score,scene_name = npz_name,modality_list=modality_list, **kwargs
                )
            )
            else:
                raise NotImplementedError()
        if data_source == 'MegaDepth' and  kwargs.get('split', False):
            return datasets
